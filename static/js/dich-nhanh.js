...
$(document).ready(async () => {
  try {
    ...
    $.get('/static/datasource/Unihan_Readings.txt').done((data) => {
      pinyinList = data.split(/\r?\n/)
                       .filter((element) => element.match(/^U\+\d+\tkMandarin/))
                       .map((element) => [
                         String.fromCodePoint(parseInt(element.split(/\t/)[0].match(/U\+(\d+)/)[1], 16)),
                         element.split(/\t/)[2]
                       ]);
  ...
  try {
    ...
    $.get('/static/datasource/ChinesePhienAmWords của thtgiang.txt')
     .done((data) => {
    ...
    await $.get('/static/datasource/TTV Translate.ChinesePhienAmWords.txt')
           .done((data) => {
             chinesePhienAmWordList = [
               ...chinesePhienAmWordList,
               ...data.split(/\r?\n/)
    ...
    await $.get('/static/datasource/QuickTranslate2020 - ChinesePhienAmWords.txt')
           .done((data) => {
             chinesePhienAmWordList = [
               ...chinesePhienAmWordList,
               ...data.split(/\r?\n/)
    ...
    await $.get('/static/datasource/Hán việt.txt')
...
$(visualViewport)
.resize((event) => $('.textarea')
.css('max-height', event.target.height - ((event.target.width < 1200 ? 23.28703703703703 : 40.33092037228542) / 100) * event.target.height + 'px'));
...
fontOptions.click(function () {
  ...
  if (!$(this).text().includes('Mặc định')) {
    $(document.body)
    .css('--opt-font-family', `${$(this).text().includes(' ') ? `'${$(this)
    .text()}'` : $(this).text()}, serif`);
...
fontSize.on('input', function () {
  ...
  $(document.body)
  .css('--opt-font-size', translator[fontSize.attr('id')] / 100);
});
fontSize.change(() => {
  ...
  if (!$(this).hasClass('disabled')) {
    ...
    sourceLangSelectOptions.each(function (index) {
      if (translators.filter($('.active'))
                     .data('id') === prevTranslator && prevSourceLanguageCode != null) {
        sourceLanguage.val(prevSourceLanguageCode);
        return false;
      } else if (prevSourceLanguageCode != null && (prevSourceLanguageName != null && $(this)
      .text()
      .replace(/Tiếng /gi, '')
      .replace(/[()]/g, '') === prevSourceLanguageName.replace(/Tiếng /gi, '')
                                                      .replace(/[()]/g, '') || $(this)
      .val()
      .toLowerCase()
      .split('_')[0] === prevSourceLanguageCode.toLowerCase()
                                               .split('_')[0] || (prevSourceLanguageName != null && $(this)
      .text()
      .includes($(this)
      .text()
      .split(' ').length === 2 && prevSourceLanguageName.split(' ').length === 2 ? prevSourceLanguageName.replace(/Tiếng /gi, '')
                                                                                                         .replace(/[()]/g, '')
                                                                                                         .split(' ')[1] : prevSourceLanguageName.replace(/Tiếng /gi, '')
                                                                                                                                                .replace(/[()]/g, '')
                                                                                                                                                .split(' ')[0]) && $(this)
      .val()
      .toLowerCase()
      .split('_')[0].split('-')[0] === prevSourceLanguageCode.toLowerCase()
                                                             .split('_')[0].split('-')[0]) || ($(this)
      .val()
      .toLowerCase()
      .split('_')[0].split('-')[0] === prevSourceLanguageCode.toLowerCase()
                                                             .split('_')[0].split('-')[0]))) {
    ...
    targetLangSelectOptions.each(function (index) {
      if (translators.filter($('.active'))
        ...
        return false;
      } else if (prevTargetLanguageCode != null && (prevTargetLanguageName != null && $(this)
      .text()
      .replace(/Tiếng /gi, '')
      .replace(/[()]/g, '') === prevTargetLanguageName.replace(/Tiếng /gi, '')
                                                      .replace(/[()]/g, '') || $(this)
      .val()
      .toLowerCase()
      .split('_')[0] === prevTargetLanguageCode.toLowerCase()
                                               .split('_')[0] || (prevTargetLanguageName != null && $(this)
      .text()
      .includes($(this)
      .text()
      .split(' ').length === 2 && prevTargetLanguageName.split(' ').length === 2 ? prevTargetLanguageName.replace(/Tiếng /gi, '')
                                                                                                         .replace(/[()]/g, '')
                                                                                                         .split(' ')[1] : prevTargetLanguageName.replace(/Tiếng /gi, '')
                                                                                                                                                .replace(/[()]/g, '')
                                                                                                                                                .split(' ')[0]) && $(this)
      .val()
      .toLowerCase()
      .split('_')[0].split('-')[0] === prevTargetLanguageCode.toLowerCase()
                                                             .split('_')[0].split('-')[0]) || ($(this)
      .val()
      .toLowerCase()
      .split('_')[0].split('-')[0] === prevTargetLanguageCode.toLowerCase()
                                                             .split('_')[0].split('-')[0]))) {
        if (translators.filter($('.active'))
                       .data('id') === Translators.DEEPL_TRANSLATOR && prevTargetLanguageCode === 'en') {
        ...
        targetLanguage.val(getDefaultTargetLanguage(translators.filter($('.active'))
                                                               .data('id')));
      }
    });
...
function loadTranslatorOptions() {
  try {
    ...
    for (let i = 0; i < options.length; i++) {
      if (options.filter(`:eq(${i})`)
                 .attr('id')
                 .startsWith('flexSwitchCheck') && options.filter(`:eq(${i})`)
                                                          .prop('checked') === true) {
        data[options.filter(`:eq(${i})`)
                    .attr('id')] = options.filter(`:eq(${i})`).prop('checked');
      } else if (options.filter(`:eq(${i})`)
                        .attr('name') != undefined && options.filter(`:eq(${i})`)
                                                             .attr('name')
                                                             .startsWith('flexRadio') && options.filter(`:eq(${i})`)
                                                                                                .prop('checked') === true) {
        data[options.filter(`:eq(${i})`)
                    .attr('name')] = options.filter(`:eq(${i})`).val();
...
async function translate(inputText, abortSignal) {
  ...
  try {
    ...
    if (!abortSignal.aborted && getGlossaryAppliedText(processText, translator === Translators.MICROSOFT_TRANSLATOR, true)
    .split(/\n/)
    ...
    if (getGlossaryAppliedText(processText, translator === Translators.MICROSOFT_TRANSLATOR, true) !== prevTranslation[0]) {
          ...
          switch (translator) {
            ...
            case Translators.VIETPHRASE:
              if (targetLanguage.val() === 'vi' && Object.keys(VietphraseData.vietphrases).length > 0) {
                translatedText = convertText(translateText, VietphraseData.vietphrases, true, flexSwitchCheckGlossary.prop('checked'), translationAlgorithm.filter(':checked')
                                                                                                                                                           .val(), multiplicationAlgorithm.filter(':checked')
                                                                                                                                                                                          .val());
      ...
      prevTranslation = [
        getGlossaryAppliedText(processText, translator === Translators.MICROSOFT_TRANSLATOR, true),
        results,
        translateTimer.text()
      ];
    } else {
...
function buildTranslatedResult(inputTexts, result, showOriginal) {
  ...
  .text(convertText(inputTexts[1], {}, false, false, VietPhraseTranslationAlgorithms.TRANSLATE_FROM_LEFT_TO_RIGHT, VietPhraseMultiplicationAlgorithm.NOT_APPLICABLE));
  ...
  try {
    if (showOriginal) {
      ...
      for (let i = 0; i < inputLines.length; i++) {
        if (i < resultLines.length) {
          if (inputLines[i + lostLineFixedAmount].trim().length === 0 && resultLines[i].trim().length > 0) {
            ...
            i--;
            continue;
          } else if (translators.filter($('.active'))
                                .data('id') === Translators.PAPAGO && resultLines[i].trim().length === 0 && inputLines[i + lostLineFixedAmount].trim().length > 0) {
            lostLineFixedAmount--;
            continue;
          }
      ...
      resultDiv.innerHTML = `<p>${resultLines.map((element) => element.trimStart())
                                             .join('</p><p>')}</p>`;
    }
  } catch (error) {
    resultDiv.innerHTML = `<p>${resultLines.map((element) => element.trimStart())
...
function convertText(inputText, data, caseSensitive, useGlossary, translationAlgorithm = VietPhraseTranslationAlgorithms.PRIORITIZE_LONG_VIETPHRASE_CLUSTERS, multiplicationAlgorithm = VietPhraseMultiplicationAlgorithm.MULTIPLICATION_BY_PRONOUNS_NAMES) {
  try {
    let glossaryEntries = Object.entries(glossary)
                                .filter(([first]) => inputText.includes(first));
      ...
      if (translationAlgorithm === VietPhraseTranslationAlgorithms.PRIORITIZE_LONG_VIETPHRASE_CLUSTERS) {
        ...
        for (let j = 0; j < chars.length; j++) {
          for (const phraseLength of phraseLengths) {
            if (useGlossary && filteredGlossaryEntries.map(([, second]) => second)
                                                      .indexOf(chars.substring(j, j + phraseLength)) !== -1) {
              ...
              if (tempWord.length > 0) {
                if (j + 1 === chars.length) {
                  tempWord.substring(0 + (tempWord.startsWith(' ') ? 1 : 0), tempWord.length - (tempWord.endsWith(' ') ? 1 : 0))
                          .split(' ')
                          .forEach((element, index, array) => phrases.push(element));
                  tempWord = '';
                  break;
                } else {
                  for (const phraseLength1 of phraseLengths) {
                    if ((useGlossary && filteredGlossaryEntries.map(([, second]) => second)
                                                               .indexOf(chars.substring(j + 1, j + 1 + phraseLength1)) !== -1) || (data.hasOwnProperty(chars.substring(j + 1, j + 1 + phraseLength1)))) {
                      tempWord.substring(0 + (tempWord.startsWith(' ') ? 1 : 0), tempWord.length - (tempWord.endsWith(' ') ? 1 : 0))
                              .split(' ')
                              .forEach((element, index, array) => phrases.push(element));
                      tempWord = '';
    ...
    return caseSensitive ? result.split(/\n/)
                                 .map((element => element.replace(/(^|\s*(?:[!\-.:;?]\s+|[''\p{Ps}\p{Pi}]\s*))(\p{Ll})/gu, (match, p1, p2) => p1 + p2.toUpperCase())))
                                 .join('\n') : result;
  } catch (error) {
    ...
    try {
      ...
      const response = await $.ajax({
        ...
        data: `text=${inputText.split(/\n/)
                               .map((sentence) => encodeURIComponent(sentence))
                               .join('&text=')}${sourceLang !== '' ? '&source_lang=' + sourceLang : ''}&target_lang=${targetLang}`,
        method: 'POST'
      });
      return response.translations.map((element) => element.text.trim())
                     .join('\n');
    } catch (error) {
      console.error('Bản dịch lỗi:', error.stack);
      throw error.toString();
...
const GoogleTranslate = {
  translateText: async function (data, inputText, sourceLanguage, targetLanguage, useGlossary = false, tc = 0) {
    try {
      const querys = useGlossary ? getGlossaryAppliedText(inputText, false, flexSwitchCheckAllowAnothers.prop('checked'))
      .split(/\n/) : inputText.split(/\n/);
      ...
      const response = await $.ajax({
        ...
        data: `q=${querys.map((sentence) => encodeURIComponent(sentence))
                         .join('&q=')}`,
        method: 'POST',
        headers: {
      ...
      return response.map((element) => ((sourceLanguage === 'auto' ? element[0] : element).includes('<i>') ? (sourceLanguage === 'auto' ? element[0] : element).split('</i> <b>')
                                                                                                                                                               .filter((element) => element.includes('</b>'))
                                                                                                                                                               .map((element) => ('<b>' + element.replace(/<i>.+/, '')))
                                                                                                                                                               .join(' ') : (sourceLanguage === 'auto' ? element[0] : element)).trim())
                     .join('\n');
    } catch (error) {
      console.error('Bản dịch lỗi:', error.stack);
      throw error.toString();
  ...
  Language: [
    {
      ...
      "rtl": "false",
      "modes": [
        {"name": "Translate web site", "value": true},
        {"name": "Translation", "value": true},
        {"name": "Translation document", "value": true},
        {"name": "Image recognition", "value": true},
        {"name": "Image object recognition", "value": true}
      ]
    },
    {
      "full_code": "sq_AL",
    ...
    },
    {
      ...
      "modes": [
        ...
        {"name": "Translation", "value": true},
        {"name": "Translate web site", "value": true},
        {"name": "Speech recognition", "value": true},
        {"name": "Translation document", "value": true},
        {"name": "Image recognition", "value": true}
      ]
    },
...
const MicrosoftTranslator = {
  ...
  Language: {
    "af": {"name": "Tiếng Afrikaans", "nativeName": "Afrikaans", "dir": "ltr"},
    "am": {"name": "Tiếng Amharic", "nativeName": "አማርኛ", "dir": "ltr"},
    "ar": {"name": "Tiếng Ả Rập", "nativeName": "العربية", "dir": "rtl"},
    "as": {"name": "Tiếng Assam", "nativeName": "অসমীয়া", "dir": "ltr"},
    "az": {
      "name": "Tiếng Azerbaijan",
      "nativeName": "Azərbaycan",
      "dir": "ltr"
    },
    "ba": {"name": "Tiếng Bashkir", "nativeName": "Bashkir", "dir": "ltr"},
    "bg": {"name": "Tiếng Bulgaria", "nativeName": "Български", "dir": "ltr"},
    "bn": {"name": "Tiếng Bangla", "nativeName": "বাংলা", "dir": "ltr"},
    "bo": {"name": "Tiếng Tây Tạng", "nativeName": "བོད་སྐད་", "dir": "ltr"},
    "bs": {"name": "Tiếng Bosnia", "nativeName": "Bosnian", "dir": "ltr"},
    "ca": {"name": "Tiếng Catalan", "nativeName": "Català", "dir": "ltr"},
    "cs": {"name": "Tiếng Séc", "nativeName": "Čeština", "dir": "ltr"},
    "cy": {"name": "Tiếng Wales", "nativeName": "Cymraeg", "dir": "ltr"},
    "da": {"name": "Tiếng Đan Mạch", "nativeName": "Dansk", "dir": "ltr"},
    "de": {"name": "Tiếng Đức", "nativeName": "Deutsch", "dir": "ltr"},
    "dsb": {
      "name": "Tiếng Hạ Sorbia",
      "nativeName": "Dolnoserbšćina",
      "dir": "ltr"
    },
    "dv": {"name": "Tiếng Divehi", "nativeName": "ދިވެހިބަސް", "dir": "rtl"},
    "el": {"name": "Tiếng Hy Lạp", "nativeName": "Ελληνικά", "dir": "ltr"},
    "en": {"name": "Tiếng Anh", "nativeName": "English", "dir": "ltr"},
    "es": {"name": "Tiếng Tây Ban Nha", "nativeName": "Español", "dir": "ltr"},
    "et": {"name": "Tiếng Estonia", "nativeName": "Eesti", "dir": "ltr"},
    "eu": {"name": "Tiếng Basque", "nativeName": "Euskara", "dir": "ltr"},
    "fa": {"name": "Tiếng Ba Tư", "nativeName": "فارسی", "dir": "rtl"},
    "fi": {"name": "Tiếng Phần Lan", "nativeName": "Suomi", "dir": "ltr"},
    "fil": {
    ...
    "fo": {"name": "Tiếng Faroe", "nativeName": "Føroyskt", "dir": "ltr"},
    "fr": {"name": "Tiếng Pháp", "nativeName": "Français", "dir": "ltr"},
    "fr-CA": {
      "name": "Tiếng Pháp (Canada)",
      "nativeName": "Français (Canada)",
      "dir": "ltr"
    },
    "ga": {"name": "Tiếng Ireland", "nativeName": "Gaeilge", "dir": "ltr"},
    "gl": {"name": "Tiếng Galician", "nativeName": "Galego", "dir": "ltr"},
    "gom": {"name": "Konkani", "nativeName": "Konkani", "dir": "ltr"},
    "gu": {"name": "Tiếng Gujarati", "nativeName": "ગુજરાતી", "dir": "ltr"},
    "ha": {"name": "Tiếng Hausa", "nativeName": "Hausa", "dir": "ltr"},
    "he": {"name": "Tiếng Do Thái", "nativeName": "עברית", "dir": "rtl"},
    "hi": {"name": "Tiếng Hindi", "nativeName": "हिन्दी", "dir": "ltr"},
    "hr": {"name": "Tiếng Croatia", "nativeName": "Hrvatski", "dir": "ltr"},
    "hsb": {
      "name": "Tiếng Thượng Sorbia",
      "nativeName": "Hornjoserbšćina",
      "dir": "ltr"
    },
    "ht": {"name": "Tiếng Haiti", "nativeName": "Haitian Creole", "dir": "ltr"},
    "hu": {"name": "Tiếng Hungary", "nativeName": "Magyar", "dir": "ltr"},
    "hy": {"name": "Tiếng Armenia", "nativeName": "Հայերեն", "dir": "ltr"},
    "id": {"name": "Tiếng Indonesia", "nativeName": "Indonesia", "dir": "ltr"},
    "ig": {"name": "Tiếng Igbo", "nativeName": "Ásụ̀sụ́ Ìgbò", "dir": "ltr"},
    "ikt": {"name": "Inuinnaqtun", "nativeName": "Inuinnaqtun", "dir": "ltr"},
    "is": {"name": "Tiếng Iceland", "nativeName": "Íslenska", "dir": "ltr"},
    "it": {"name": "Tiếng Italy", "nativeName": "Italiano", "dir": "ltr"},
    "iu": {"name": "Tiếng Inuktitut", "nativeName": "ᐃᓄᒃᑎᑐᑦ", "dir": "ltr"},
    "iu-Latn": {
      "name": "Inuktitut (Latin)",
      "nativeName": "Inuktitut (Latin)",
      "dir": "ltr"
    },
    "ja": {"name": "Tiếng Nhật", "nativeName": "日本語", "dir": "ltr"},
    "ka": {"name": "Tiếng Georgia", "nativeName": "ქართული", "dir": "ltr"},
    "kk": {"name": "Tiếng Kazakh", "nativeName": "Қазақ Тілі", "dir": "ltr"},
    "km": {"name": "Tiếng Khmer", "nativeName": "ខ្មែរ", "dir": "ltr"},
    "kmr": {
      "name": "Tiếng Kurd (Bắc)",
      "nativeName": "Kurdî (Bakur)",
      "dir": "ltr"
    },
    "kn": {"name": "Tiếng Kannada", "nativeName": "ಕನ್ನಡ", "dir": "ltr"},
    "ko": {"name": "Tiếng Hàn", "nativeName": "한국어", "dir": "ltr"},
    "ku": {
      "name": "Tiếng Kurd (Trung)",
      "nativeName": "Kurdî (Navîn)",
      "dir": "rtl"
    },
    "ky": {"name": "Tiếng Kyrgyz", "nativeName": "Кыргызча", "dir": "ltr"},
    "ln": {"name": "Tiếng Lingala", "nativeName": "Lingála", "dir": "ltr"},
    "lo": {"name": "Tiếng Lào", "nativeName": "ລາວ", "dir": "ltr"},
    "lt": {"name": "Tiếng Litva", "nativeName": "Lietuvių", "dir": "ltr"},
    "lug": {"name": "Ganda", "nativeName": "Ganda", "dir": "ltr"},
    "lv": {"name": "Tiếng Latvia", "nativeName": "Latviešu", "dir": "ltr"},
    "lzh": {
      "name": "Chinese (Literary)",
      "nativeName": "中文 (文言文)",
      "dir": "ltr"
    },
    "mai": {"name": "Tiếng Maithili", "nativeName": "Maithili", "dir": "ltr"},
    "mg": {"name": "Tiếng Malagasy", "nativeName": "Malagasy", "dir": "ltr"},
    "mi": {"name": "Tiếng Maori", "nativeName": "Te Reo Māori", "dir": "ltr"},
    "mk": {"name": "Tiếng Macedonia", "nativeName": "Македонски", "dir": "ltr"},
    "ml": {"name": "Tiếng Malayalam", "nativeName": "മലയാളം", "dir": "ltr"},
    "mn-Cyrl": {
    ...
    "mr": {"name": "Tiếng Marathi", "nativeName": "मराठी", "dir": "ltr"},
    "ms": {"name": "Tiếng Mã Lai", "nativeName": "Melayu", "dir": "ltr"},
    "mt": {"name": "Tiếng Malta", "nativeName": "Malti", "dir": "ltr"},
    "mww": {"name": "Tiếng H’Mông", "nativeName": "Hmong Daw", "dir": "ltr"},
    "my": {"name": "Tiếng Miến Điện", "nativeName": "မြန်မာ", "dir": "ltr"},
    "nb": {
      "name": "Tiếng Na Uy (Bokmål)",
      "nativeName": "Norsk Bokmål",
      "dir": "ltr"
    },
    "ne": {"name": "Tiếng Nepal", "nativeName": "नेपाली", "dir": "ltr"},
    "nl": {"name": "Tiếng Hà Lan", "nativeName": "Nederlands", "dir": "ltr"},
    "nso": {
    ...
    "or": {"name": "Tiếng Odia", "nativeName": "ଓଡ଼ିଆ", "dir": "ltr"},
    "otq": {
      "name": "Tiếng Querétaro Otomi",
      "nativeName": "Hñähñu",
      "dir": "ltr"
    },
    "pa": {"name": "Tiếng Punjab", "nativeName": "ਪੰਜਾਬੀ", "dir": "ltr"},
    "pl": {"name": "Tiếng Ba Lan", "nativeName": "Polski", "dir": "ltr"},
    "prs": {"name": "Tiếng Dari", "nativeName": "دری", "dir": "rtl"},
    "ps": {"name": "Tiếng Pashto", "nativeName": "پښتو", "dir": "rtl"},
    "pt": {
    ...
    "ro": {"name": "Tiếng Romania", "nativeName": "Română", "dir": "ltr"},
    "ru": {"name": "Tiếng Nga", "nativeName": "Русский", "dir": "ltr"},
    ...
    "sd": {"name": "Tiếng Sindhi", "nativeName": "سنڌي", "dir": "ltr"},
    "si": {"name": "Tiếng Sinhala", "nativeName": "සිංහල", "dir": "ltr"},
    "sk": {"name": "Tiếng Slovak", "nativeName": "Slovenčina", "dir": "ltr"},
    "sl": {"name": "Tiếng Slovenia", "nativeName": "Slovenščina", "dir": "ltr"},
    "sm": {"name": "Tiếng Samoa", "nativeName": "Gagana Sāmoa", "dir": "ltr"},
    "sn": {"name": "Tiếng Shona", "nativeName": "chiShona", "dir": "ltr"},
    "so": {"name": "Tiếng Somali", "nativeName": "Soomaali", "dir": "ltr"},
    "sq": {"name": "Tiếng Albania", "nativeName": "Shqip", "dir": "ltr"},
    "sr-Cyrl": {
    ...
    "sv": {"name": "Tiếng Thụy Điển", "nativeName": "Svenska", "dir": "ltr"},
    "sw": {"name": "Tiếng Swahili", "nativeName": "Kiswahili", "dir": "ltr"},
    "ta": {"name": "Tiếng Tamil", "nativeName": "தமிழ்", "dir": "ltr"},
    "te": {"name": "Tiếng Telugu", "nativeName": "తెలుగు", "dir": "ltr"},
    "th": {"name": "Tiếng Thái", "nativeName": "ไทย", "dir": "ltr"},
    "ti": {"name": "Tiếng Tigrinya", "nativeName": "ትግር", "dir": "ltr"},
    "tk": {"name": "Tiếng Turkmen", "nativeName": "Türkmen Dili", "dir": "ltr"},
    "tlh-Latn": {
    ...
    "to": {"name": "Tiếng Tonga", "nativeName": "Lea Fakatonga", "dir": "ltr"},
    "tr": {"name": "Tiếng Thổ Nhĩ Kỳ", "nativeName": "Türkçe", "dir": "ltr"},
    "tt": {"name": "Tiếng Tatar", "nativeName": "Татар", "dir": "ltr"},
    "ty": {"name": "Tiếng Tahiti", "nativeName": "Reo Tahiti", "dir": "ltr"},
    "ug": {"name": "Tiếng Uyghur", "nativeName": "ئۇيغۇرچە", "dir": "rtl"},
    "uk": {"name": "Tiếng Ukraina", "nativeName": "Українська", "dir": "ltr"},
    "ur": {"name": "Tiếng Urdu", "nativeName": "اردو", "dir": "rtl"},
    "uz": {"name": "Tiếng Uzbek", "nativeName": "Uzbek (Latin)", "dir": "ltr"},
    "vi": {"name": "Tiếng Việt", "nativeName": "Tiếng Việt", "dir": "ltr"},
    "xh": {"name": "Tiếng Xhosa", "nativeName": "isiXhosa", "dir": "ltr"},
    "yo": {"name": "Tiếng Yoruba", "nativeName": "Èdè Yorùbá", "dir": "ltr"},
    "yua": {
