...
$('#imageURL').change(function () {
    ...
    img.onload = function () {
        ...
        recognize(document.querySelector('#recognizeImage').toDataURL(), $('#recognizeLangsSelect').val().join('+'), {tessedit_pageseg_mode: $('#recognizeLangsSelect').val().join('+').includes('_vert') ? PSM.AUTO : PSM.SINGLE_BLOCK, tessedit_ocr_engine_mode: OEM.LSTM_ONLY})
...
$('.option').on('change', () => localStorage.setItem('recognizer', JSON.stringify({
    langs: $('#recognizeLangsSelect').val(),
    grayscale: $('#flexCheckGrayscale').prop('checked'),
    threshold: $('#flexCheckThreshold').prop('checked')
})));
...
async function recognize(image, langs, parameters) {
    ...
    return worker.recognize(image).finally(async () => await worker.terminate());
}