...
const VOS = {
  ...
  tonePattern: '[aoôơuư][ìỉĩíị]|[ae][òỏõóọ]|[aâêiyươ][ùũủúụ]|[aâ][ỳỷỹýỵ]|[iu][àảãáạ]|[òỏõóọ][aăeo]|[ùũủúụ]yê|[ùũủúụ][âêyơô]|[ỳỷỹýỵìỉĩíị]ê|[ừửữứự]ơ|g[ìỉĩíị][aăeêoôơuư]|q[ùũủúụ][ai]',
...
function vosOaoeuy (sample): string {
  const Cf = VOS.Cf
  ...
  for (let i = 0; i < wrong.length; i++) {
    // Replace wrong
    sample = sample.replaceAll(new RegExp(`(?<=(?:\\P{L}|^)(?:${Cf})?)${wrong[i]}`, 'gu'), right[i])
  }
  return sample
}

function reversedVosOaoeuy (sample): string {
  const Cf = VOS.Cf
  ...
  for (let i = 0; i < right.length; i++) {
    // Replace right
    sample = sample.replaceAll(new RegExp(`(?<=(?:\\P{L}|^)(?:${Cf})?)${right[i]}`, 'gu'), wrong[i])
  }
  return sample
}

function vosFixTonemarkPosition (sample): string {
  const replacing = {}

  // Process incorrect tone placements
  for (const combination of [...sample.matchAll(VOS.tonePatternRegex)].map(match => match[0]) as string[]) {
    let consonant = ''
    let tonedCharacter = ''
    ...
    let position: number = combination.split('').reverse().join('').indexOf(tonedCharacter)

    if (consonant.length > 0) position += 1
