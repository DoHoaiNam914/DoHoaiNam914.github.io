...
const glossaryCounter = $('#glossaryCounter');
...
const glossaryName = $('#glossaryName');
...
upperCaseButtons.on('click', function () {
  if (targetEntry.val().length > 0) {
    targetEntry.val(targetEntry.val().split(' ').map((word, index) => word = $(this).data('amount') === '#' ? word.charAt(0).toUpperCase() + word.slice(1) : (index < $(this).data('amount') ? word.charAt(0).toUpperCase() + word.slice(1) : word.toLowerCase())).join(' '));
  }
});
...
function loadGlossary() {
  ...
  if (glossaryArray.length > 0) {
    glossary = Object.fromEntries(glossaryArray.sort((a, b) => /\p{Lu}/u.test(b[1][0]) - /\p{Lu}/u.test(a[1][0]) || a[0].startsWith(b[0]) || a[1].localeCompare(b[1], 'vi', {ignorePunctuation: true}) || a[0].localeCompare(b[0], 'vi', {ignorePunctuation: true}) || b[0].length - a[0].length));
  ...
  glossaryList.html(entrySelect.innerHTML);
