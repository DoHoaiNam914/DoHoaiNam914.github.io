...
upperCaseButtons.on('click', function () {
    if (targetEntry.val().length > 0) targetEntry.val(targetEntry.val().split(' ').map((word, index) => word = $(this).data('amount') === '#' ? word.charAt(0).toUpperCase() + word.slice(1) : (index < $(this).data('amount') ? word.charAt(0).toUpperCase() + word.slice(1) : word.toLowerCase())).join(' '));
});
