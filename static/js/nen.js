    ...
    if (this.value !== Colors.WHITE) {
        if ($(document.documentElement).attr('style') != undefined) {
            Object.values(Colors).forEach(function (code) {
                if ($(document.documentElement).attr('style').includes(code)) $(document.documentElement).attr('style', $(document.documentElement).attr('style').replace(new RegExp(`\s?${code}-background;`), ''));
            });
        ...
        Object.values(Colors).forEach(function (code) {
            if ($(document.documentElement).attr('style').includes(code)) $(document.documentElement).attr('style', $(document.documentElement).attr('style').replace(new RegExp(`\s?${code}-background;`, 'g'), ''));
        });
