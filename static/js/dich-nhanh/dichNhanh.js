...
$(document).ready(() => {
  ...
  $.ajax({
    ...
    let hanvietList = data.split('\n').map((element) => element.split('\t')).filter(([first, second]) => /^\p{Script=Hani}+$/u.test(first) && second.includes('Hán Việt:')).map(([first, second]) => [first, second.replaceAll('<span class="east"> </span>', ' ').match(/Hán Việt:(?: |)[^<]*/g).filter((element) => element.replace(/Hán Việt: ?/, '').length > 0)[0].replace(/Hán Việt: ?/, '').split(/[,;] ?/)[0].toLowerCase()]);
