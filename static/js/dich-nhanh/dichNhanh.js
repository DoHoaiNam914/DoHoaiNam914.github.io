...
$glossaryListSelect.change(function onChange() {
  ...
  if (isLoaded && $sourceEntryInput.val().length > 0 && (Object.hasOwn(glossary[$(this).val()], $sourceEntryInput.val()) || window.confirm('Bạn có muốn chuyển đổi lại chứ?'))) {
    if (Object.hasOwn(glossary[$(this).val()], $sourceEntryInput.val())) {
      $targetEntryTextarea.val(glossary[$(this).val()][$sourceEntryInput.val()]).trigger('input');
    } else {
...
$addButton.click(() => {
  ...
  if ($glossaryListSelect.val() === 'vietPhrasePhu' && Object.hasOwn(glossary.vietPhrase, $sourceEntryInput.val().trim())) delete glossary.vietPhrase[$sourceEntryInput.val().trim()];
...
$removeButton.on('click', () => {
  if (Object.hasOwn(glossary[$glossaryListSelect.val()], $sourceEntryInput.val())) {
