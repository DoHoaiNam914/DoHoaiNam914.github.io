...
$(document).ready(async () => {
  ...
  try {
    ...
    })).split('\n').filter((element) => !element.startsWith('#')).map((element) => element.split('\t')).filter(([first, second]) => /^\p{Script=Hani}+$/u.test(first) && /Hán Việt: *[^<]*/.test(second.replaceAll('<span class="east"> </span>', ' '))).map(([first, second]) => [first, [...second.replaceAll('<span class="east"> </span>', ' ').matchAll(/Hán Việt: *[^<]*/g)].map((element) => element.filter((element) => element.length > 0))]).filter(([__, second]) => second.length > 0).map(([first, second]) => [first, second.flat().map((element) => element.normalize().replace(/Hán Việt: */, ''))]).map(([first, second]) => [first, second.filter((element) => element.length > 0).sort((a, b) => b.split(/[,;] */).length - a.split(/[,;] */).length)[0].trimEnd().split(/[,;] */).toReversed()[0]]);
