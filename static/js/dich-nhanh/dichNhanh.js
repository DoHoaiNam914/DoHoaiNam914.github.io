...
$(document).ready(() => {
  ...
  $.ajax({
    ...
    let hanvietList = data.split('\n').map((element) => (!element.startsWith('#') ? element.split('\t') : element)).filter((a) => typeof a !== 'string' && /^\p{Script=Hani}+$/u.test(a[0]) && [...a[1].replaceAll('<span class="east"> </span>', ' ').matchAll(/Hán Việt: *[^<]*/g)].filter((b) => b.replace(/Hán Việt: */, '').length > 0).length > 0).map(([first, second]) => [first, second.replaceAll('<span class="east"> </span>', ' ').matchAll(/Hán Việt: *[^<]*/g).filter((element) => element.replace(/Hán Việt: */, '').length > 0)[0].replace(/Hán Việt: */, '').split(/[,;] */)[0].trim().toLowerCase()]);
