...
\`\`\`` : ''}`,
            ...
            } : null,
            {
              ...
              parts: [
                {
                  text: `Translate the text in VĂN BẢN GỐC into Vietnamese. In that text, regardless of whether it is a title or content, all must be translated. ${terminologies.length > 0 || names.length > 0 ? `Make sure to enhance the accuracy of the translation with ${terminologies.length > 0 ? 'the terms listed in BẢNG TRA CỨU THUẬT NGỮ ' : ''}${names.length > 0 ? `${terminologies.length > 0 ? 'and ' : ''}the proper names listed in BẢNG TRA CỨU TÊN RIÊNG ` : ''}. ` : ''}Reference the raw translation in BẢN DỊCH THÔ for accuracy and consistency. Your translations must convey all the content in the original text and cannot involve explanations or other unnecessary information. Please ensure that the translated text is natural for native speakers with correct grammar and proper word choices. Your output must only contain the translated text and cannot include explanations or other information. Absolutely do not format the output text.`,
                },
              ],
            },
            {
