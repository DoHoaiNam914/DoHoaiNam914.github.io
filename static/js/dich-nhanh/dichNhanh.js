...
const translate = async function translateContentInTextarea(controller = new AbortController()) {
  ...
  try {
    ...
    if ([Translators.DEEPL_TRANSLATE, Translators.WEBNOVEL_TRANSLATE].every((element) => $activeTranslator.val() !== element) && targetLanguage.startsWith('vi')) {
...
$(document).ready(async () => {
  ...
  try {
    ...
    })).split('\n').filter((element) => !element.startsWith('#')).map((element) => element.split('\t')).filter(([first, second]) => /^\p{Script=Hani}+$/u.test(first) && /Hán Việt: *[^<]*/.test(second.replaceAll('<span class="east"> </span>', ' '))).map(([first, second]) => [first, [...second.replaceAll('<span class="east"> </span>', ' ').matchAll(/Hán Việt: *[^<]*/g)].map((element) => element.filter((element) => element.trim().length > 0))]).filter(([__, second]) => second.length > 0).map(([first, second]) => [first, second.flat().map((element) => element.normalize().replace(/Hán Việt: */, ''))]).map(([first, second]) => [first, second.sort((a, b) => b.split(/[,;] */).length - a.split(/[,;] */).length)[0].trimEnd().split(/[,;] */).toReversed()[0]]);
...
$translateEntryButtons.click(async function onClick() {
  ...
  if ($translateButton.text() !== 'Huỷ' && text.length > 0) {
    ...
    try {
      ...
      if ([Translators.COCCOC_EDU_TRANSLATE, Translators.DEEPL_TRANSLATE].every((element) => activeTranslator !== element) && targetLanguage.startsWith('vi')) translator.result = await polishTranslation(artificialIntelligence ?? 'none', text, translator.result, nameEnabled != null && Boolean(nameEnabled) !== false);
