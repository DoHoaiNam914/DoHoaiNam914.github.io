          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: 'Translate the text in #original-text into Vietnamese. Use the names from the #name-dictionary and the terms from the #glossary to enhance the translation. When writing Japanese names, use Hepburn Romanization. For Chinese names, use Sino-Vietnamese. Please refer to the raw translation in #raw-translation. Your translations must convey all the content in the original text in #original-text line by line and cannot involve explanations or other unnecessary information. Ensure to strictly maintain the same number of lines as the original text in #original-text. Please ensure that the translated text is natural for native speakers with correct grammar and proper word choices. Standardize the use of I/Y for the main vowel and the placement of tone marks in syllables with -oa/-oe/-uy in the translated text. Your output must only contain the translated text and cannot include explanations or other information.',
                },
              ],
            },
            {
...
<pre type="text/plain" id="raw-translation">
  ...
  $sourceEntryInput.autocomplete({
    ...
    source: (request, response) => {
      ...
      autocompleteTimeout = setTimeout(() => {
        ...
        response($.grep(/[\p{Script=Hani}\p{Script=Hira}\p{Script=Kana}]/u.test(request.term) || request.term.length >= 2 ? autocompleteGlossarySource : [], (elementOfArray) => elementOfArray.label.split('=').some((element, index) => ((index === 0 && element !== request.term) || /[\p{Script=Hani}\p{Script=Hira}\p{Script=Kana}]/u.test(element) || element.length >= 2) && (matcher.test(element) || matcher.test(element.normalize('NFKD').replaceAll(/\p{Mn}/gu, '').replaceAll('đ', 'd').replaceAll('Đ', 'D'))))).slice(0, 50));
      }, 300);
    },
...
$translatorDropdown.find('.dropdown-item').click(function onClick() {
  ...
  currentTranslator = structuredClone(translators)[activeTranslator];
...
$translateEntryButtons.click(async function onClick() {
  ...
  if ($translateButton.text() !== 'Huỷ' && text.length > 0) {
    ...
    let translator = structuredClone(translators)[activeTranslator];
