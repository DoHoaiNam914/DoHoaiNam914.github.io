...
class Vietphrase {
  ...
  translatePrioritizeLongVietPhraseClusters(targetLanguage, data, inputText) {
    ...
    try {
      ...
      let dataArray = data.concat(this.data.pronoun).toSorted((a, b) => b[0].length - a[0].length);
  ...
  translateFromLeftToRight(targetLanguage, data, inputText) {
    ...
    try {
      let dataArray = data.concat(this.data.pronoun);
      let nameArray = this.data.name;
  ...
  async translateText(__, targetLanguage, inputText, translationAlgorithm, multiplicationAlgorithm, prioritizeNameOverVietPhrase, addDeLeZhao, autocapitalize, data, nameEnabled, glossary = [], useToneMarks = true) {
    ...
    this.data.hanViet = new Map(this.data.SinoVietnameses.concat(this.data.hanViet).filter(([first], __, array) => !array[first] && (array[first] = 1), {}));
