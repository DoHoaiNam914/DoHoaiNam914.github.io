...
class WebnovelTranslate extends Translator {
  ...
  async translateText(text, targetLanguage, sourceLanguage = this.DefaultLanguage.SOURCE_LANGUAGE) {
    try {
      ...
      if (text.split('\n').length > 1) console.log('DEBUG:', responses.map((element) => element.responseJSON[0].filter(([__, second]) => second != null).map(([first, second]) => [second, first, CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? first.replace(/^\s+/, '').replaceAll(/\s*(?:\| \|{2,3}|\|{3} \||\|(?:(?: \|){1,3}| )|\|{1,4})\s*/g, '\n') : first.replace(/^\s+/, '').replaceAll(/\s*\\ ?n\s*/gi, '\n')]).map(([first, second, third]) => [first, second, [...first.matchAll(CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? /\|{4}/g : /\\n/g)].length, [...third.matchAll(/\n/g)].length]).map(([first, second, third, fourth]) => (third >= fourth ? [first, second] : [first, second, third, fourth]))));
      this.result = responses.map((a) => a.responseJSON[0].filter(([__, second]) => second != null).map(([first, second]) => [second, CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? first.replace(/^\s+/, '').replaceAll(/\s*(?:\| \|{2,3}|\|{3} \||\|(?:(?: \|){1,3}| )|\|{1,4})\s*/g, '\n') : first.replace(/^\s+/, '').replaceAll(/\s*\\ ?n\s*/gi, '\n')]).map(([first, second]) => second.concat('\n'.repeat([...first.matchAll(CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? /\|{4}/g : /\\n/g)].length - [...second.matchAll(/\n/g)].length))).join('')).join('\n');
