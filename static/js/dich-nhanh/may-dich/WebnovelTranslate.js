...
class WebnovelTranslate extends Translator {
  ...
  async translateText(text, targetLanguage, sourceLanguage = this.DefaultLanguage.SOURCE_LANGUAGE) {
    try {
      ...
      if (text.split('\n').length > 1) console.log('DEBUG:', responses.map((element) => element.responseJSON[0].filter(([__, second]) => second != null).map(([first, second]) => [second, first, CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? first.replaceAll(/\s*(?:[ |]{2,4}|\|{1,4})\s*/g, '\n').concat('\n'.repeat([...second.matchAll(/\|{4}/g)].length - [...first.replaceAll(/\s*(?:[ |]{2,4}|\|{1,4})\s*/g, '\n').matchAll(/\n/g)].length)) : first.replaceAll(/\s*\\ ?n\s*/gi, '\n')])));
      this.result = responses.map((a) => a.responseJSON[0].filter(([__, second]) => second != null).map(([first, second]) => (CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? first.replaceAll(/\s*(?:[ |]{2,4}|\|{1,4})\s*/g, '\n').concat('\n'.repeat([...second.matchAll(/\|{4}/g)].length - [...first.replaceAll(/\s*(?:[ |]{2,4}|\|{1,4})\s*/g, '\n').matchAll(/\n/g)].length)) : first.replaceAll(/\s*\\ ?n\s*/gi, '\n'))).join('')).join('\n');
