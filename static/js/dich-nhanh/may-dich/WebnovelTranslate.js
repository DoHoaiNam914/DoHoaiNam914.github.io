...
class WebnovelTranslate extends Translator {
  /** http://translate.google.com/translate_a/l?client=gtx */
  ...
  async translateText(text, targetLanguage, sourceLanguage = this.DefaultLanguage.SOURCE_LANGUAGE) {
    try {
      ...
      if (text.split('\n').length > 1) console.log('DEBUG:', responses.map((element) => element.responseJSON[0].filter(([__, second]) => second != null).map(([first, second]) => [second, first, CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? first.replace(/^\s+/, '').replaceAll(/\s*(?:\| \|{3}|(?:\| ){1,3}|\|{1,4})\s*/g, '\n') : first.replace(/^\s+/, '').replaceAll(/\s*\\ ?n\s*/gi, '\n')]).map(([first, second, third]) => [first, second, [...first.matchAll(CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? /\|{4}/g : /\\n/g)].length, [...third.matchAll(/\n/g)].length]).filter(([__, ___, third, fourth]) => third > fourth)));
      this.result = responses.map((a) => a.responseJSON[0].filter(([__, second]) => second != null).map(([first, second]) => [second, CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? first.replace(/^\s+/, '').replaceAll(/\s*(?:\| \|{3}|(?:\| ){1,3}|\|{1,4})\s*/g, '\n') : first.replace(/^\s+/, '').replaceAll(/\s*\\ ?n\s*/gi, '\n')]).map(([first, second]) => second.concat('\n'.repeat([...first.matchAll(CJ_LANGUAGE_CODE_LIST.some((b) => sourceLanguage === b) ? /\|{4}/g : /\\n/g)].length - [...second.matchAll(/\n/g)].length))).join('')).join('\n');
