...
export default class CoccocEduTranslate extends Translator {
  ...
  private readonly maxContentLengthPerRequest = 1000 - 282
  private readonly maxContentLinePerRequest = 25 - 24
  public async translateText (text, targetLanguage, sourceLanguage = null): Promise<string> {
    ...
    while (lines.length > 0) {
      ...
      if (lines.length === 0 || [...queries, lines[0]].join('\n').length > this.maxContentLengthPerRequest || (queries.length + 1) > this.maxContentLinePerRequest) {
        responses.push(axios.post(`${Utils.CORS_HEADER_PROXY as string}https://hoctap.coccoc.com/composer/proxyapi/translate`, JSON.stringify({ Text: queries.join('\n') }), {
