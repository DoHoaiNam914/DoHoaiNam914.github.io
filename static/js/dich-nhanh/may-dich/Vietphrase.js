...
class Vietphrase extends Translator {
  ...
  async translateText(text, targetLanguage, glossary, options) {
    ...
    hanViet = SinoVietnameses.filter(([__, second]) => !/\p{Script_Extensions=Hani}/u.test(second)).map(([first, second]) => [first, second.toLowerCase()]).concat(SinoVietnameses.filter(([__, second]) => /\p{Script_Extensions=Hani}/u.test(second)).map(([first, second]) => [first, second.split(/(?:)/u).map((element) => hanVietMap[element]).join(' ')]), glossary.hanViet).filter(function filter([first]) {
      return !this[first] && (this[first] = 1);
    }, {});
