    ...
    async translateText(text, targetLanguage, options = { model: 'gpt-4o-mini', temperature: 1, maxTokens: 0, topP: 1, nomenclature: [], splitChunkEnabled: false }) {
        ...
        while (queues.length > 0) {
            ...
            if (queues.length === 0 || (splitChunkEnabled && [...queries, queues[0]].join('\n').length > this.maxContentLengthPerRequest)) {
                ...
                const MESSAGE = PROMPT_INSTRUCTIONS.includes('Nomenclature Mapping Table') ? `<|begin_of_text|><|start_header_id|>nomenclature_mapping_table<|end_header_id|>\n\nsource\ttarget\n${nomenclature.join('\n')}<|eot_id|><|start_header_id|>text<|end_header_id|>\n\n${query}<|eot_id|>` : query;
