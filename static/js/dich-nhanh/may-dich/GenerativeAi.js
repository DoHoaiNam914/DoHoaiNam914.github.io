...
Current date: ${currentDate.split('/').reverse().join('-')}
...
`,
    ...
    async mainAnthropic(options, promptInstructions, message) {
        ...
        const dayStringWithSuffix = (day) => day === 1 ? `${day}st` : (day === 2 ? `${day}nd` : (day === 3 ? `${day}rd` : `${day}th`));
        switch (body.model) {
            case 'claude-3-5-sonnet-20241022':
            case 'claude-3-5-sonnet-latest':
            case 'claude-3-5-sonnet-20240620':
                body.system = `The assistant is Claude, created by Anthropic.
The current date is ${currentDate.replace(/^(\d{1,2}) (\S+)/, (match, p1, p2) => `${p2}, ${dayStringWithSuffix(parseInt(p1))}`)}.
