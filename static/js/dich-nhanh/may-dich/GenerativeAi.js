...
export default class GenerativeAi extends Translator {
    ...
    async runHfInference(model, instructions, message) {
        ...
        if (['meta-llama/Llama-3.2-3B-Instruct', 'meta-llama/Llama-3.2-1B-Instruct', 'microsoft/Phi-3-mini-4k-instruct'].some(element => model === element))
            chatCompletionInput.max_tokens = 4096 - (model.startsWith('google') || model.startsWith('meta-llama') ? getEncoding('gpt2').encode(`${instructions}${message}`).length : 0);
