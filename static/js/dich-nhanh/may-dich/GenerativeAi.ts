  ...
  public async translateText (text, targetLanguage: string, options: { sourceLanguage: string | null, model?: string, temperature?: number, topP?: number, topK?: number, systemPrompt?: string, tone?: string, domain?: string, customPrompt?: string, dictionary?: string[][] } = { sourceLanguage: null }): Promise<string> {
    ...
    switch (systemPrompt) {
      case 'Professional': {
        ...
        const originalLang: string | undefined = targetLanguage === 'English' ? sourceLanguage?.toUpperCase() : sourceLanguage?.replace(/^(Chinese \()(Simplified|Traditional)/, (_match, p1: string, p2: string) => `${p1}${p2.toLowerCase()}`)
        const destLang: string = targetLanguage === 'English' ? targetLanguage.toUpperCase() : targetLanguage.replace(/^(Chinese \()(Simplified|Traditional)/, (_match, p1: string, p2: string) => `${p1}${p2.toLowerCase()}`)
