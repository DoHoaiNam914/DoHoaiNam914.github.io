  ...
  public async translateText (text, targetLanguage: string, options: { [key: string]: any } = { model: 'gpt-4o-mini', temperature: 1, maxTokens: 0, topP: 1, nomenclature: [], splitChunkEnabled: false }): Promise<string> {
    ...
    while (queues.length > 0) {
      ...
      if (queues.length === 0 || (splitChunkEnabled && [...queries, queues[0]].join('\n').length > this.maxContentLengthPerRequest)) {
        ...
        const MESSAGE = PROMPT_INSTRUCTIONS.includes('Nomenclature Mapping Table') ? `[NOMENCLATURE_MAPPING_TABLE] source\ttarget\n${nomenclature.join('\n')}[/NOMENCLATURE_MAPPING_TABLE][TEXT] ${query}[/TEXT]` : query
