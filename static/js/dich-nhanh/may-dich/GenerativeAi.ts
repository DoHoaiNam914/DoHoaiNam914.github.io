...
export default class GenerativeAi extends Translator {
  ...
  public async runOpenai (model: string, instructions: string, message: string): Promise<string> {
    ...
    switch (model) {
      ...
      default:
        ...
        else if (['gpt-4', 'gpt-4-0613', 'gpt-4-0314'].some(element => model === element)) maxCompletionTokens = 8192 - encodingForModel(model).encode(`${instructions}${message}`).length
        break
    }
  ...
  public async runHfInference (model: string, instructions: string, message: string): Promise<string> {
    ...
    if (['meta-llama/Llama-3.2-3B-Instruct', 'google/gemma-2-9b-it', 'meta-llama/Llama-3.2-1B-Instruct', 'microsoft/Phi-3-mini-4k-instruct'].some(element => model === element)) chatCompletionInput.max_tokens = 4096 - encodingForModel('gpt-4o').encode(model.startsWith('meta-llama') ? `<|begin_of_text|>\n${chatCompletionInput.messages.map(({ content, role }) => `<|start_header_id|>${role}<|end_header_id|>\n\n${content}\n<|eot_id|>`).join('\n')}` : chatCompletionInput.messages.map(({ content, role }) => `${role}\n${content}`).join('\n')).length
    else if (model.startsWith('google') || model.startsWith('meta-llama')) chatCompletionInput.max_tokens = chatCompletionInput.max_tokens - encodingForModel('gpt-4o').encode(model.startsWith('meta-llama') ? `<|begin_of_text|>\n${chatCompletionInput.messages.map(({ content, role }) => `<|start_header_id|>${role}<|end_header_id|>\n\n${content}\n<|eot_id|>`).join('\n')}` : chatCompletionInput.messages.map(({ content, role }) => `${role}\n${content}`).join('\n')).length
