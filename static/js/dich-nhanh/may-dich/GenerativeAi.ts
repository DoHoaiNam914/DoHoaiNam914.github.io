  ...
  public async translateText (text, targetLanguage: string, options: { sourceLanguage: string | null, model?: string, temperature?: number, topP?: number, topK?: number, systemPrompt?: string, tone?: string, domain?: string, customPrompt?: string, dictionary?: string[][] } = { sourceLanguage: null }): Promise<string> {
    ...
    switch (systemPrompt) {
      case 'Advanced': {
        ...
        const originalLang: string = targetLanguage === 'English' ? sourceLanguage.toUpperCase() : sourceLanguage.toLowerCase().replace(/^(\p{Ll})/, (_match, p1) => p1.toUpperCase())
        const destLang: string = targetLanguage === 'English' ? targetLanguage.toUpperCase() : targetLanguage.toLowerCase().replace(/^(\p{Ll})/, (_match, p1) => p1.toUpperCase())
        const DOMAIN_MAP = {
