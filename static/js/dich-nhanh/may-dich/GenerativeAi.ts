...
`,
  ...
  public async runGoogleGenerativeAI (options, promptInstructions, message): Promise<string> {
    ...
    if (/^gemini-1\.5-[^-]+-001$/.test(modelParams.model)) generationConfig.topK = 64
...
The current date is ${currentDate.replace(/^(\d{1,2}) (\S+)/, (match, p1, p2) => `${p2 as string}, ${dayStringWithSuffix(parseInt(p1))}`)}.
