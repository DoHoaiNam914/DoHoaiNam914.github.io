...
export default class MicrosoftTranslator extends Translator {
    ...
    async setToneAndFetchData(tone) {
        this.tone = tone;
        await this.instance.get('/translator').then(response => {
            this.IG = response.data.match(/IG:"([A-Z0-9]+)"/)[1];
            const [, [, casualAndFormalIid], [, standardIid]] = [...response.data.matchAll(/data-iid="(translator\.\d+)"/g)];
            switch (this.tone) {
                case 'Casual':
                case 'Formal': {
            ...
            const [, key, token] = response.data.match(/var params_AbusePreventionHelper = \[([0-9]+),"([^"]+)",[^\]]+\];/);
            this.key = key;
            this.token = token;
        }).catch(({ data }) => {
    ...
    async translateText(text, targetLanguage, sourceLanguage = null) {
        if (this.IG == null || this.IID == null || this.token == null || this.key == null)
            await this.setToneAndFetchData(this.tone);
        ...
        while (lines.length > 0) {
            ...
            if (lines.length === 0 || [...queries, lines[0]].join('\n').length > this.maxContentLengthPerRequest) {
                const queryText = encodeURIComponent(queries.join('\n'));
                responses.push(this.instance.post('/ttranslatev3', `&fromLang=${sourceLanguage ?? this.DefaultLanguage.SOURCE_LANGUAGE}&to=${targetLanguage}&token=${this.token}&key=${this.key}${this.tone !== 'Standard' ? `&tone=${this.tone}&text=${queryText}` : `&text=${queryText}&tryFetchingGenderDebiasedTranslations=true`}`, {
