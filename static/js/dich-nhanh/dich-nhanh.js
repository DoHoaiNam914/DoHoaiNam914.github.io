          ...
          switch (optionId) {
            case 'font': {
              data.font = selectedValue.text()
                .includes('Mặc định') ? selectedValue.text()
                  .match(/(.+) \(/)[1] : selectedValue.text();
              break;
            }
            case 'translator': {
...
function loadAllQuickTranslatorOptions() {
  ...
  $options.each((index) => {
    ...
    switch (optionType) {
      ...
      case OptionTypes.CHECK:
      case OptionTypes.SWITCH: {
        option.attr('checked', quickTranslateStorage[optionId])
          .change();
        break;
      }
      case OptionTypes.RANGE: {
      ...
      case OptionTypes.DROPDOWN: {
        ...
        switch (optionId) {
          case 'font': {
            values.filter(`:contains(${quickTranslateStorage[optionId]})`)
              .click();
            break;
          }
          case 'translator': {
            values.filter(`[data-id="${quickTranslateStorage[optionId]}"]`)
              .click();
            break;
          }
          // no default
  ...
  if (glossaryEntries.length > 0) {
    ...
    switch ($glossaryType.val()) {
      ...
      case GlossaryType.VIETPHRASE: {
        glossaryData = glossaryEntries.map((element) => (element.length > 2 ? element.splice(2, glossary.length - 2) : element).join('='))
          .join('\n');
...
function applyGlossaryToText(text, translator = '') {
  ...
  if (glossaryEntries.length > 0) {
    ...
    const glossaryLengths = [...glossaryEntries.map(([first]) => first.length), 1].sort((a, b) => b - a)
...
function buildResult(inputText, result) {
  try {
    ...
    if ($showOriginalTextSwitch.prop('checked')) {
      ...
      for (let i = 0; i < inputLines.length; i += 1) {
        if (i + lostLineFixedNumber < resultLines.length) {
          if (inputLines[i + lostLineFixedNumber].length === 0 && resultLines[i].length > 0) {
            ...
            i -= 1;
          } else if ($translatorOptions.filter($('.active'))
            .data('id') === Translators.PAPAGO && resultLines[i].length === 0 && inputLines[i + lostLineFixedNumber].length > 0) {
            lostLineFixedNumber -= 1;
          } else {
      ...
      resultDiv.innerHTML = `<p>${resultLines.map(Utils.convertTextToHtml)
        .join('</p><p>')}</p>`;
    }
...
async function translateTextarea() {
  ...
  const translatorOption = $translatorOptions.filter($('.active'))
    .data('id');
  const sourceLanguage = $sourceLanguageSelect.val();
  const targetLanguage = $targetLanguageSelect.val();

  const languagePairs = $('#language-pairs-select')
    .val();
    ...
    if (Object.keys(lastSession).length > 0 && lastSession.inputText === (glossaryEnabled && (translatorOption === Translators.VIETPHRASE ? targetLanguage === 'vi' : sourceLanguage.split('-')[0].toLowerCase() === glossaryLanguageSource && targetLanguage.split('-')[0].toLowerCase() === glossaryLanguageTarget) ? applyGlossaryToText(inputText) : inputText) && lastSession.translatorOption === translatorOption && lastSession.sourceLanguage === sourceLanguage && lastSession.targetLanguage === targetLanguage) {
      ...
      switch (translatorOption) {
        ...
        case Translators.VIETPHRASE: {
          translator = await new Vietphrase(vietphraseData, $translationAlgorithmRadio.filter('[checked]')
            .val(), $multiplicationAlgorithmRadio.filter('[checked]')
            .val(), $('#ttvtranslate-mode-switch')
            .prop('checked'), glossaryEnabled && targetLanguage === 'vi', glossary, $prioritizeNameOverVietphraseCheck.prop('checked'), true);
          break;
        }
        default: {
...
function getSourceLanguageSelectOptions(translator) {
  ...
  switch (translator) {
    ...
    case Translators.GOOGLE_TRANSLATE: {
      Object.entries(GoogleTranslate.SOURCE_LANGUAGES)
    ...
    case Translators.PAPAGO: {
      Object.entries(Papago.SOURCE_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
    ...
    case Translators.VIETPHRASE: {
      Object.entries(Vietphrase.SOURCE_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
    ...
    default: {
      Object.entries(MicrosoftTranslator.FROM_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
...
function getTargetLanguageSelectOptions(translator) {
  ...
  switch (translator) {
    ...
    case Translators.GOOGLE_TRANSLATE: {
      Object.entries(GoogleTranslate.TARGET_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
    ...
    case Translators.PAPAGO: {
      Object.entries(Papago.TARGET_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
    ...
    case Translators.VIETPHRASE: {
      Object.entries(Vietphrase.TARGET_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
    ...
    default: {
      Object.entries(MicrosoftTranslator.TO_LANGUAGES)
        .forEach(([languageCode]) => {
          const option = document.createElement('option');
    ...
    switch (translatorOption) {
      ...
      case Translators.VIETPHRASE: {
        translator = await new Vietphrase(vietphraseData, $translationAlgorithmRadio.filter('[checked]')
          .val(), $multiplicationAlgorithmRadio.filter('[checked]')
          .val(), $('#ttvtranslate-mode-switch')
          .prop('checked'), glossaryEnabled, glossary, $prioritizeNameOverVietphraseCheck.prop('checked'));
...
$(document)
  .ready(async () => {
    ...
    try {
      ...
      await $.ajax({
        ...
        .done((data) => {
          pinyinList = data.split(/\r?\n/)
            .filter((element) => element.startsWith('U+'))
            .map((element) => element.substring(2)
              .split(/\t/))
            .map(([first, second]) => [String.fromCodePoint(parseInt(first, 16)), second]);
      ...
      await $.ajax({
        ...
        .done((data) => pinyinList === [...pinyinList, ...data.split(/\r?\n/)
          ...
          .map(([first, second]) => [first, second.split('ǀ')[0]])
          .filter(([first]) => !Object.prototype.hasOwnProperty.call(vietphraseData.pinyins, first))]);
    ...
    try {
      let chinesePhienAmWordList = [...specialSinovietnameseData.map(([a, b, c]) => [a, (Object.fromEntries(specialSinovietnameseData.filter(([, d]) => !/\p{sc=Hani}/u.test(d))
        .map(([d, e, f]) => [d, f ?? e]))[b] ?? c ?? b).split(', ')[0].toLowerCase()]), ...window.hanData.names.map(([first, second]) => [first, second.split(',')
      ...
      await $.ajax({
        ...
        .done((data) => {
          chinesePhienAmWordList = [...chinesePhienAmWordList, ...data.split(/\r?\n/)
            .map((element) => element.split('='))
            .filter(([first]) => !Object.prototype.hasOwnProperty.call(vietphraseData.chinesePhienAmWords, first))];
      ...
      await $.ajax({
        ...
        .done((data) => {
          let vietphraseList = [...data.split(/\r?\n/)
            .map((element) => element.split('='))
            .filter((element) => element.length === 2)
            .map(([first, second]) => [first, second.split('/')[0].split('|')[0]]), ...Object.entries(vietphraseData.chinesePhienAmWords)];
...
$(window)
  .on('keydown', (event) => {
    if ($(document.activeElement)
      .is('body') && $resultTextarea.is(':visible')) {
      switch (event.key) {
        case 'Home': {
...
$translateButton.on('click', function () {
  ...
  switch ($(this)
    ...
    default: {
      ...
      $('#action-navbar .paste-button')
        .addClass('disabled');
      $resultTextarea.html($resultTextarea.html()
        .split(/<br>|<\/p><p>/)
...
$copyButtons.on('click', async function () {
  ...
  if ($(this)
    .data('target') === '#glossary-data-list') {
...
$('#glossary-management-button')
  .on('mousedown', () => {
    $glossaryDataList.val('')
      .change();
    $sourcePairInput.val(getSelectedTextOrActiveElementText())
      .trigger('input');
  });
$options.change(function () {
  const optionId = getOptionId($(this)
    .attr('name') != null ? $(this)
      .attr('name') : $(this)
      .attr('id'));
  const optionType = getOptionType($(this)
    .attr('name') != null ? $(this)
      .attr('name') : $(this)
      .attr('id'));
  ...
  if (optionType === OptionTypes.CHECK || optionType === OptionTypes.SWITCH) {
    quickTranslateStorage[optionId] = $(this)
      .prop('checked');
  } else {
    quickTranslateStorage[optionId] = $(this)
      .val();
  ...
  if ($(this)
    .hasClass('quick-translate-option')) {
    lastSession = {};
    $retranslateButton.click();
  }
});
$fontOptions.click(function () {
  ...
  if (!$(this)
    ...
    .includes('Mặc định')) {
    $(document.body)
      .css('--opt-font-family', `${$(this)
        .text()
        .includes(' ') ? `'${$(this)
          .text()}'` : $(this)
          .text()}, ${$(this)
        .data('additional-fonts') != null && $(this)
        .data('additional-fonts').length > 0 ? `${$(this)
          .text()
          .includes(' ') ? `'${$(this)
            .data('additional-fonts')}'` : $(this)
            .data('additional-fonts')}, serif` : 'serif'}`);
  } else if ($(this)
    .text() === 'Phông chữ hệ thống') {
    $(document.body)
      .css('--opt-font-family', 'var(--system-font-family)');
  } else {
    $(document.body)
      .css('--opt-font-family', 'Georgia, serif');
  }

  quickTranslateStorage.font = $(this)
    .text()
    .includes('Mặc định') ? $(this)
      .text()
      .match(/(.+) \(/)[1] : $(this)
      .text();
  localStorage.setItem('dich_nhanh', JSON.stringify(quickTranslateStorage));
...
$fontSizeRange.on('input', function () {
  $('#font-size-display')
    .val(parseInt($(this)
      .val(), 10));
  $(document.body)
    .css('--opt-font-size', `${parseInt($(this)
      .val(), 10) / 100}rem`);
  quickTranslateStorage[getOptionId($(this)
    .attr('id'))] = parseInt($(this)
    .val(), 10);
});
$('#font-size-display')
  .on('change', function () {
    const maybeValueIsBiggerThanMaxValue = $(this)
      .val() > parseInt($fontSizeRange.attr('max'), 10) ? $fontSizeRange.attr('max') : $(this)
        .val();
    $fontSizeRange.val($(this)
      .val() < parseInt($fontSizeRange.attr('min'), 10) ? $fontSizeRange.attr('min') : maybeValueIsBiggerThanMaxValue)
      .change();
  });
...
$lineSpacingRange.on('input', function () {
  $('#line-spacing-display')
    .val(parseInt($(this)
      .val(), 10));
  $(document.body)
    .css('--opt-line-height', `${1 + ((0.5 * parseInt($(this)
      .val(), 10)) / 100)}em`);
  quickTranslateStorage[getOptionId($(this)
    .attr('id'))] = parseInt($(this)
    .val(), 10);
...
$('#line-spacing-display')
  .on('change', function () {
    const maybeValueIsBiggerThanMaxValue = $(this)
      .val() > parseInt($lineSpacingRange.attr('max'), 10) ? $lineSpacingRange.attr('max') : $(this)
        .val();
    $lineSpacingRange.val($(this)
      .val() < parseInt($lineSpacingRange.attr('min'), 10) ? $lineSpacingRange.attr('min') : maybeValueIsBiggerThanMaxValue)
      .change();
  });
...
$alignmentSettingsSwitch.change(function () {
  $(document.body)
    .css('--opt-text-align', $(this)
      .prop('checked') ? 'justify' : 'start');
  quickTranslateStorage[getOptionId($(this)
    .attr('id'))] = $(this)
    .prop('checked');
  localStorage.setItem('dich_nhanh', JSON.stringify(quickTranslateStorage));
...
$translatorOptions.click(function () {
  ...
  $(this)
    .addClass('active');
  updateLanguageSelect($(this)
    .data('id'), quickTranslateStorage.translator);
  quickTranslateStorage.translator = $(this)
    .data('id');
  localStorage.setItem('dich_nhanh', JSON.stringify(quickTranslateStorage));
  $retranslateButton.click();
});
$vietphrasesInput.on('change', function () {
  ...
  reader.onload = function () {
    let vietphraseList = this.result.split(/\r?\n/)
      .map((element) => element.split($vietphrasesInput.prop('files')[0].type === 'text/tab-separated-values' ? '\t' : '='))
      .filter((element) => element.length === 2)
...
$glossaryInput.on('change', function () {
  ...
  reader.onload = function () {
    ...
    $glossaryName.val($glossaryInput.prop('files')[0].name.split('.')
      .slice(0, $glossaryInput.prop('files')[0].name.split('.').length - 1)
      .join('.'));
    reloadGlossaryEntries();
  };
  reader.readAsText($(this)
...
$('.dropdown-toggle')
  .on('click', () => $('.dropdown-scroller')
    .prop('scrollTop', 0));
$('#source-pair-dropdown')
...
$('.define-button')
  .on('click', function () {
    if ($sourcePairInput.val().length > 0) {
      window.open($(this)
        .data('href')
        .replace('{0}', encodeURIComponent(($sourcePairInput.val()
          .substring($sourcePairInput.prop('selectionStart'), $sourcePairInput.prop('selectionEnd')) || $sourcePairInput.val()).substring(0, 30)
          .trim())));
...
$('.upper-case-button')
  .on('click', function () {
    if ($targetPairInput.val().length > 0) {
      $targetPairInput.val($targetPairInput.val()
        .split(' ')
        .map((element, index) => {
          const maybeIndexIsSmallerThanAmount = index < $(this)
            .data('amount') ? element.charAt(0)
              .toUpperCase() + element.slice(1) : element.toLowerCase();
          return element === $(this)
            .data('amount') === '#' ? element.charAt(0)
              .toUpperCase() + element.slice(1) : maybeIndexIsSmallerThanAmount;
        })
        .join(' '));
    }
  });
...
$addButton.on('click', () => {
  glossary[$sourcePairInput.val()
    .trim()] = $targetPairInput.val()
    .trim();
  reloadGlossaryEntries();
...
$glossaryDataList.change(function () {
  if (Object.prototype.hasOwnProperty.call(glossary, $(this)
    .val())) {
    $sourcePairInput.val($(this)
      .val())
      .trigger('input');
    $removeButton.removeClass('disabled');
  } else {
